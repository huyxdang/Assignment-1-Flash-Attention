...
def _flash_attention_forward_kernel(
    ...
    for start_n in range(0, SEQ_LEN, BLOCK_N):
        ...
        # 1) New row-wise max across this tile
        m_ij  = tl.max(s_ij, axis=1)                 # [BLOCK_M]
        m_new = tl.maximum(m_i, m_ij)                # [BLOCK_M]

        # 2) Rescale previous accumulators by alpha = exp(m_i - m_new)
        #    (use exp2 since s_ij is in log2-base scaling)
        alpha = tl.exp2(m_i - m_new)                 # [BLOCK_M]
        acc   = acc * alpha[:, None]
        l_i   = l_i * alpha

        # 3) Probabilities for current tile: p_ij = exp(s_ij - m_new)
        p_ij = tl.exp2(s_ij - m_new[:, None])        # [BLOCK_M, BLOCK_N]

        # 4) Update numerator accumulator with current tile: acc += p_ij @ V_j
        #    Cast to float32 for stable accumulation.
        acc += tl.dot(p_ij.to(tl.float32), v_block.to(tl.float32))

        # 5) Update denominator: l_i += sum_j p_ij
        l_i += tl.sum(p_ij, axis=1)

        # 6) Advance running max
        m_i = m_new
