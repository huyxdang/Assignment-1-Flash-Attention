...
def weighted_row_sum_kernel(
    ...
    # 2. Create a pointer to the start of the current row in X.
    row_start_ptr = X_ptr + row_idx * N_COLS

    # 3. Pointer for the output vector Y.
    output_ptr = Y_ptr + row_idx

    # 4. Initialize an accumulator (float32 for stability).
    accumulator = tl.zeros((BLOCK_SIZE,), dtype=tl.float32)

    # 5. Iterate over the columns in blocks of BLOCK_SIZE.
    ...
    # 6. Reduce accumulator to scalar
